...
async def send_welcome(message: Message):
    ...
    try:
        ...
        logging.info(f"[send_welcome] user={message.from_user.id} отправлен файл {WELCOME_MEDIA_PATH}")
    except FileNotFoundError:
        logging.warning(f"Файл приветственного медиа не найден: {WELCOME_MEDIA_PATH}")
        await message.answer(WELCOME_TEXT, reply_markup=kb)
        logging.info(f"[send_welcome] user={message.from_user.id} медиа-файла нет, отправлен текст+меню")
    except Exception as e:
        # Раньше любая ДРУГАЯ ошибка (не FileNotFoundError) вылетала бы наружу
        # и пользователь не получал вообще НИЧЕГО, даже без сообщения об ошибке.
        logging.exception(f"[send_welcome] user={message.from_user.id} неожиданная ошибка при отправке медиа: {e}")
        await message.answer(WELCOME_TEXT, reply_markup=kb)
        logging.info(f"[send_welcome] user={message.from_user.id} медиа не отправилось из-за ошибки, но текст+меню ушли")
