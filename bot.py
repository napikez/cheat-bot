...
# Картинка/гифка приветствия задаётся ТОЛЬКО через админ-панель бота
# (кнопка "🟢 Фото/гифка на /start" -> прислать фото/GIF) и хранится в БД как file_id.
...
class SubscriptionMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler, event: TelegramObject, data: dict):
        user = data.get("event_from_user")
        if user is None:
            logging.info("[SubMW] event_from_user отсутствует, пропускаем без проверки")
            return await handler(event, data)

        logging.info(f"[SubMW] user={user.id} event={type(event).__name__} data={getattr(event, 'data', None) if isinstance(event, CallbackQuery) else getattr(event, 'text', None)}")

        if is_admin(user.id):
            logging.info(f"[SubMW] user={user.id} это админ, пропускаем проверку")
            return await handler(event, data)

        state: FSMContext | None = data.get("state")
        current_state = await state.get_state() if state else None
        if current_state in _SPONSOR_FLOW_STATES:
            logging.info(f"[SubMW] user={user.id} в состоянии {current_state}, пропускаем проверку")
            return await handler(event, data)

        if isinstance(event, CallbackQuery) and event.data in _SUBSCRIPTION_BYPASS_CALLBACKS:
            logging.info(f"[SubMW] user={user.id} callback {event.data} в списке bypass, пропускаем")
            return await handler(event, data)
        ...
        has_grant = await get_unused_grant(user.id)
        if has_grant:
            logging.info(f"[SubMW] user={user.id} есть неиспользованный grant -> уводим в сценарий добавления спонсорской ссылки, МИМО обычного меню")
            return await handler(event, data)
        ...
        if not ok:
            logging.info(f"[SubMW] user={user.id} не подписан на всё -> отправляем subscription prompt")
            try:
                await send_subscription_prompt(event, missing)
            except Exception as e:
                logging.exception(f"Не удалось отправить запрос подписки {user.id}: {e}")
            return
        logging.info(f"[SubMW] user={user.id} подписан на всё (или спонсоров нет) -> пропускаем в обработчик")
        return await handler(event, data)
...
async def start_with_payload(message: Message, command: CommandObject):
    token = command.args
    logging.info(f"[start_with_payload] user={message.from_user.id} token={token}")
    cheat = await get_cheat_by_token(token)
    if not cheat:
        logging.info(f"[start_with_payload] user={message.from_user.id} токен {token} не найден в базе читов")
...
async def send_welcome(message: Message):
    logging.info(f"[send_welcome] user={message.from_user.id} формируем приветствие+меню")
    kb = user_main_keyboard()

    # Фото/гифка, загруженные админом через кнопку "🟢 Фото/гифка на /start"
    # (админ-панель -> присылает фото/GIF боту -> сохраняется в settings как file_id).
    # Локальный файл welcome.png/welcome.gif больше не используется — если админ
    # ничего не загрузил, /start просто присылает текст без картинки.
    ...
    if media_type and media_id:
        try:
            ...
            logging.info(f"[send_welcome] user={message.from_user.id} отправлено сохранённое медиа ({media_type})")
            return
        except Exception as e:
            # file_id мог протухнуть (например, админ удалил исходное сообщение в Telegram)
            # — в этом случае просто откатываемся на текст, а не молчим.
            logging.warning(f"Не удалось отправить сохранённое приветственное медиа: {e}")

    await message.answer(WELCOME_TEXT, reply_markup=kb)
    logging.info(f"[send_welcome] user={message.from_user.id} медиа не задано/не отправилось, отправлен текст+меню")


@router.message(CommandStart())
async def start_plain(message: Message, state: FSMContext):
    logging.info(f"[start_plain] user={message.from_user.id} дошли до самого хендлера")
    if is_admin(message.from_user.id):
        logging.info(f"[start_plain] user={message.from_user.id} админ -> показываем админ-панель")
    ...
    if await get_unused_grant(message.from_user.id):
        logging.info(f"[start_plain] user={message.from_user.id} есть grant -> сценарий добавления спонсора вместо меню")
    ...
    logging.info(f"[start_plain] user={message.from_user.id} обычный пользователь без grant -> send_welcome")
